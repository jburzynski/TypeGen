...
move TypeGen.2.1.1.nupkg nuget -force

if (Test-Path "local-nuget-path.txt") {
  $localNuGetPath = Get-Content "local-nuget-path.txt"
  copy nuget\TypeGen.2.1.1.nupkg $localNuGetPath
}
...
move TypeGen.DotNetCli.2.1.1.nupkg nuget-dotnetcli -force

if (Test-Path "local-nuget-path.txt") {
  $localNuGetPath = Get-Content "local-nuget-path.txt"
  copy nuget-dotnetcli\TypeGen.DotNetCli.2.1.1.nupkg $localNuGetPath
}
